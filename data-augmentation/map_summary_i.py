...
`<splits-dir>/progress/<split>.summary_i.progress.jsonl` so a crash
mid-run resumes from where it left off on restart. Failed generations
(None) are not persisted, so they get retried.
...
def _sidecar_path(split_path: Path) -> Path:
    progress_dir = split_path.parent / "progress"
    progress_dir.mkdir(parents=True, exist_ok=True)
    return progress_dir / (split_path.name + ".summary_i.progress.jsonl")
