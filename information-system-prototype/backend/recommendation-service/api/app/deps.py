"""FastAPI dependencies: the current principal.

recommendation-api has no users table — every recommendation endpoint is scoped
to the caller's own `user_id`. Supports two authentication modes (`AUTH_MODE`):
