"""SQLAlchemy models for `recommendationdb` (pgvector).
...
Two semantic-vector tables, both 768-dimensional and L2-normalized:
  • profession_vectors — career-SBERT embedding of the profession text, plus
    denormalized fields the recommendation-api filters/sorts on;
  • user_vectors — the CareerRNN output for the user's career history.

The mapped ESCO skills of a user are stored by user-service (not here).
"""
...
class ProfessionVector(Base):
    ...
    # Denormalized fields for recommendation-api filters/sorting.
