...
Adds the column `esco_skills_cosine_hybrid_070` to each split CSV.

Per-resume progress is mirrored to a sidecar `<split>.progress.jsonl`
so a crash mid-run resumes from the next unfinished row on restart.

Install:
    pip install -U "sentence-transformers>=3.2" pandas numpy
...
Usage:
    python map_esco_cosine_hybrid.py
    python map_esco_cosine_hybrid.py --threshold 0.7 --label-weight 0.6 --concat-weight 0.4
"""
...
OUTPUT_COLUMN = "esco_skills_cosine_hybrid_adaptive_060_v3"
...
def load_generic_topic_nouns(cache_path: Path) -> frozenset[str]:
    """Read the empirically-built generic-topic-noun set from JSON. The
    cache schema matches what map_esco_gemma.py writes: a top-level
    "skills" array of single-word inputs."""
    if not cache_path.exists():
        print(
            f"  WARNING: generic-noun cache {cache_path} not found — "
            f"GENERIC branch of get_hybrid_weight will be empty.",
            file=sys.stderr,
        )
        return frozenset()
    ...
    return frozenset(s.lower().strip() for s in skills if isinstance(s, str) and s.strip())

# Resume-level batching for query encoding. Each chunk gathers all
# distinct, non-adjective skills across `embed_chunk_rows` resumes and
# encodes them in one `encode([...])` call. `embed_batch_size` is the
# inner per-step batch size used by sentence-transformers — it controls
# GPU/CPU utilization, while `embed_chunk_rows` controls how many rows'
# worth of skills are made visible to that batching at once.
DEFAULT_EMBED_CHUNK_ROWS = 32
...
def decode_skills(value) -> list[str]:
...
def build_context(row, current_skill: str, all_skills: list[str]) -> str:
    """
    Compose a richer per-skill context from the row's fields.
    Field priority (greedy-assembly, capped at CONTEXT_CHARS):
      summary → experience (title: description pairs) → other skills.
    """
    parts: list[str] = []

    summary = row.get("summary")
    if isinstance(summary, str) and summary.strip():
        parts.append(f"Summary: {summary.strip()[:SUMMARY_CHARS]}")

    experiences: list[str] = []
    n_exp = row.get("number_of_experiences")
    if isinstance(n_exp, (int, float)) and not pd.isna(n_exp):
        for i in range(int(n_exp)):
            t = row.get(f"title_{i}")
            if not (isinstance(t, str) and t.strip()):
                continue
            entry = t.strip()
            d = row.get(f"description_{i}")
            if isinstance(d, str) and d.strip():
                entry += f": {d.strip()[:DESC_CHARS_EACH]}"
            experiences.append(entry)

    if experiences:
        parts.append("Experience: " + " | ".join(experiences[:MAX_EXPERIENCES]))

    others = [s for s in all_skills if s != current_skill]
    if others:
        parts.append(f"Other skills: {', '.join(others[:MAX_OTHER_SKILLS])}")

    return ". ".join(parts)[:CONTEXT_CHARS]


def _sidecar_path(split_path: Path) -> Path:
    return split_path.parent / (split_path.name + ".progress.jsonl")


def _load_progress(sidecar_path: Path) -> dict[str, Optional[str]]:
    """identifier -> json-encoded mapped list (or None for resumes with no skills)."""
    ...
    with open(sidecar_path, encoding="utf-8") as f:
        for line in f:
            ...
            if ident is None:
                continue
            progress[str(ident)] = rec.get("mapped")
    return progress


def _record_progress(sidecar_f, identifier: str, mapped_str: Optional[str]) -> None:
    sidecar_f.write(json.dumps(
        {"identifier": identifier, "mapped": mapped_str},
        ensure_ascii=False,
    ) + "\n")
    sidecar_f.flush()


def _row_identifier(idx, row) -> str:
...
def enrich_split(
    ...
    *,
    threshold: float = COSINE_THRESHOLD,
    domain_filter_enabled: bool = True,
    embed_chunk_rows: int = DEFAULT_EMBED_CHUNK_ROWS,
    ...
    uri_to_label = index.catalog.uri_to_label

    sidecar_f = open(sidecar_path, "a", encoding="utf-8")
    try:
        pbar = tqdm(
            total=len(rows), desc=split_path.name,
            unit="resume", leave=True,
        )
        for chunk_start in range(0, len(rows), embed_chunk_rows):
            chunk = rows[chunk_start: chunk_start + embed_chunk_rows]

            # Pass 1: classify each row, collect distinct non-adjective
            # skills across the whole chunk for one batched encode call.
            entries: list[tuple] = []
            chunk_query_pos: dict[str, int] = {}
            chunk_queries: list[str] = []
            for idx, row in chunk:
                ident = _row_identifier(idx, row)
                if ident in progress:
                    entries.append(("resume", ident, progress[ident]))
                    continue
                skills = decode_skills(row.get("skills"))
                if not skills:
                    entries.append(("empty", ident))
                    continue
                unique = list(dict.fromkeys(skills))
                for s in unique:
                    if is_adjective_fragment(s):
                        continue
                    if short_circuit_uri(s) is not None:
                        # Short-circuited (e.g. language override) — no
                        # embedding needed; resolved in Pass 3 directly.
                        continue
                    if s not in chunk_query_pos:
                        chunk_query_pos[s] = len(chunk_queries)
                        chunk_queries.append(s)
                entries.append(("process", ident, row, unique))

            # Pass 2: one encode() call per chunk (vs. one per skill before).
            chunk_query_embs = encode_queries(
                chunk_queries, embed_model, embed_batch_size,
            ) if chunk_queries else None

            # Pass 3: per-row mapping, in original order, using precomputed
            # embeddings — preserves sidecar progress semantics row-by-row.
            for entry in entries:
                kind = entry[0]
                if kind == "resume":
                    mapped_str = entry[2]
                    out_col.append(mapped_str)
                    n_resumed += 1
                    pbar.update(1)
                    continue
                if kind == "empty":
                    _, ident = entry
                    out_col.append(None)
                    _record_progress(sidecar_f, ident, None)
                    pbar.update(1)
                    continue

                _, ident, row, unique = entry
                mapped: list[dict] = []
                seen_uris: set[str] = set()
                for skill in unique:
                    n_skills += 1
                    if is_adjective_fragment(skill):
                        continue
                    override_uri = short_circuit_uri(skill)
                    if override_uri is not None:
                        uri = override_uri
                    else:
                        assert chunk_query_embs is not None
                        query_emb = chunk_query_embs[chunk_query_pos[skill]]
                        context = build_context(row, skill, unique)
                        uri = map_skill(
                            skill, context, query_emb,
                            index,
                            top_k,
                            threshold=threshold,
                            domain_filter_enabled=domain_filter_enabled,
                            identifier=ident, log_file=log_file,
                        )
                    if uri is None or uri in seen_uris:
                        continue
                    seen_uris.add(uri)
                    mapped.append({"uri": uri, "label": uri_to_label.get(uri, "")})
                    n_mapped += 1

                mapped_str = json.dumps(mapped, ensure_ascii=False) if mapped else None
                out_col.append(mapped_str)
                _record_progress(sidecar_f, ident, mapped_str)
                pbar.set_postfix_str(
                    f"{n_mapped}/{n_skills} skills, {n_resumed} resumed"
                )
                pbar.update(1)
    ...
    df[OUTPUT_COLUMN] = out_col
    df.to_csv(split_path, index=False)
    print(
        f"  {split_path.name}: done — {n_mapped}/{n_skills} skills mapped, "
        f"{n_resumed} rows resumed from sidecar in {time.time()-t0:.0f}s",
        file=sys.stderr,
    )
...
def main():
    ...
    ap.add_argument("--skills-csv", type=Path,
                    default=Path("data/input/skills_en.csv"),
                    help="Path to ESCO skills_en.csv")
    ...
    ap.add_argument("--top-k", type=int, default=DEFAULT_TOP_K,
                    help="Candidates retained after the domain filter "
                         "(top-1 of these is the final pick).")
    ap.add_argument("--threshold", type=float, default=COSINE_THRESHOLD,
                    help="Hybrid score threshold for keeping the top-1 candidate.")
    ap.add_argument("--generic-set-cache", type=Path,
                    default=Path("data/output/generic_topic_nouns.json"),
                    help="JSON cache populating GENERIC_TOPIC_NOUNS — these "
                         "skills get the description-heavy weight branch in "
                         "get_hybrid_weight. Missing file -> empty set.")
    ap.add_argument("--failure-log", type=Path,
                    default=Path("data/output/map_esco_cosine_hybrid_adaptive_events.jsonl"),
                    help="JSONL of non-trivial outcomes (currently: "
                         "domain-filter drops/keeps). Pass /dev/null to disable.")
    ap.add_argument("--no-domain-filter", action="store_true",
                    help="Disable the candidate-side domain filter.")
    ap.add_argument("--embed-chunk-rows", type=int,
                    default=DEFAULT_EMBED_CHUNK_ROWS,
                    help="How many resume rows to gather before issuing one "
                         "batched encode() call for their distinct skills.")
    ap.add_argument("--embed-batch-size", type=int,
                    default=DEFAULT_EMBED_BATCH_SIZE,
                    help="Inner batch size used by sentence-transformers "
                         "for both the corpus and per-chunk query encodes.")
    args = ap.parse_args()

    print(f"Loading generic-topic-noun set from {args.generic_set_cache}...",
          file=sys.stderr)
    global GENERIC_TOPIC_NOUNS
    GENERIC_TOPIC_NOUNS = load_generic_topic_nouns(args.generic_set_cache)
    print(
        f"  {len(GENERIC_TOPIC_NOUNS)} generic-topic-nouns loaded; "
        f"{len(CONCRETE_TOOLS_SET)} concrete tools curated.",
        file=sys.stderr,
    )

    print(f"Loading ESCO catalog from {args.skills_csv}...", file=sys.stderr)
    catalog = load_esco_catalog(args.skills_csv)
    print(
        f"  {len(catalog.unique_uris)} concepts, "
        f"{len(catalog.label_texts)} label entries, "
        f"{len(catalog.concat_texts)} concat docs",
        file=sys.stderr,
    )
    ...
    print("Embedding ESCO label corpus...", file=sys.stderr)
    t0 = time.time()
    label_emb = embed_corpus(
        catalog.label_texts, embed_model, batch_size=args.embed_batch_size,
    )
    print(
        f"  done in {time.time()-t0:.1f}s, shape={label_emb.shape}",
        file=sys.stderr,
    )

    print("Embedding ESCO concat corpus...", file=sys.stderr)
    t0 = time.time()
    concat_emb = embed_corpus(
        catalog.concat_texts, embed_model, batch_size=args.embed_batch_size,
    )
    print(
        f"  done in {time.time()-t0:.1f}s, shape={concat_emb.shape}",
        file=sys.stderr,
    )

    blacklist_uri_indices = resolve_blacklist_indices(
        catalog.unique_uris, HARD_BLACKLIST,
    )
    print(
        f"  Hard blacklist: {len(blacklist_uri_indices)}/{len(HARD_BLACKLIST)} "
        f"URIs resolved to catalog indices.",
        file=sys.stderr,
    )

    known_uris = set(catalog.unique_uris)
    missing_overrides = [
        k for k, v in SHORT_CIRCUIT_OVERRIDES.items() if v not in known_uris
    ]
    if missing_overrides:
        print(
            f"  WARNING: {len(missing_overrides)} SHORT_CIRCUIT_OVERRIDES "
            f"URI(s) not found in catalog: {', '.join(sorted(missing_overrides))}",
            file=sys.stderr,
        )
    print(
        f"  Short-circuit overrides: {len(SHORT_CIRCUIT_OVERRIDES)} keys "
        f"-> {len(set(SHORT_CIRCUIT_OVERRIDES.values()))} distinct URIs "
        f"(no embedding/threshold/domain-filter applied for these inputs).",
        file=sys.stderr,
    )
    index = EscoIndex(
        catalog=catalog,
        ...
        blacklist_uri_indices=blacklist_uri_indices,
    )

    print(
        f"Enriching splits in {args.splits_dir} (threshold={args.threshold}, "
        f"top_k={args.top_k}, adaptive weights via get_hybrid_weight: "
        f"concrete={HYBRID_WEIGHTS_CONCRETE}, "
        f"generic={HYBRID_WEIGHTS_GENERIC}, "
        f"default={HYBRID_WEIGHTS_DEFAULT})...",
        file=sys.stderr,
    )
    args.failure_log.parent.mkdir(parents=True, exist_ok=True)
    log_file = open(args.failure_log, "a", encoding="utf-8")
    print(f"  Event log: {args.failure_log}", file=sys.stderr)
    try:
        domain_filter_enabled = not args.no_domain_filter
        if not domain_filter_enabled:
            print(
                "Domain filter disabled (--no-domain-filter); all candidates "
                "are eligible.",
                file=sys.stderr,
            )
        for name in SPLIT_FILES:
            enrich_split(
                args.splits_dir / name, embed_model, index, args.top_k,
                threshold=args.threshold,
                domain_filter_enabled=domain_filter_enabled,
                embed_chunk_rows=args.embed_chunk_rows,
